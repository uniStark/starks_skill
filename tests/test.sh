...
run_contract_checks() {
  ...
  assert_contains "$readme" "work-conserving" "English README documents work-conserving PM scheduling"
  assert_contains "$readme" "status board" "English README documents the user-visible status board"
  assert_contains "$readme_zh" "持续补位" "Chinese README documents continuous PM slot refill"
  assert_contains "$readme_zh" "进度看板" "Chinese README documents the user-visible progress board"
