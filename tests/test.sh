...
run_contract_checks() {
  ...
  assert_contains "$readme" "Safe Ready work starts as soon as a slot opens" "English README promises work-conserving PM scheduling"
  assert_contains "$readme" "The PM posts a truthful status board" "English README promises a truthful user-visible board"
  assert_contains "$readme_zh" "安全的 Ready 工作会在槽位释放后立即持续补位" "Chinese README promises continuous PM slot refill"
  assert_contains "$readme_zh" "主 PM 会展示真实进度看板" "Chinese README promises a truthful user-visible board"
