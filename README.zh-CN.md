...
- **可选 Obsidian 记忆层**——有实质进展时，把项目总结沉淀到知识库；未配置则优雅跳过。
...
| `STARKS_REVIEW_MODEL` | 跨模型互审在**另一端**使用的模型 | 同上 |
...
- **完整**——跨多文件 / 架构级 / 行为大改 / 明显不确定性。走完整流程：
  ...
  1. **拷问需求**——一次一问、多选优先，挖出隐藏假设、边界条件与成功标准。
  2. **起草方案**——收口需求并做轻量任务拆解。
  3. **呈现方案 + 用户定夺**——把方案交给用户三选：**A 直接开干 / B 先让另一端模型互审再定 / C 修改方案**。仅当选 B 才跑跨模型互审，整合修订版后回到本步重新定夺。互审不自动触发，也不闷头跳过。
  4. **PM 派并行子代理**——把互不依赖的部分并行派发，关键决策由你把关。
  5. **两阶段审查**——先查规格合规，再查代码质量；不过回炉。
  6. **完成门禁**——当场跑出验证证据才能宣称「完成 / 通过」。
  7. **记忆收尾（可选）**——若配置了记忆目录且本次有实质进展，把项目总结写入知识库。
