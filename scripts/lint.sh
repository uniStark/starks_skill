...
if [[ -f "$SKILL" ]]; then
  anchors=("HARD-GATE" "STARKS_CROSS_REVIEW" "跨模型互审" "references/runtime.md" "references/pm-orchestration.md" "prompts/spec-review.md" "prompts/code-review.md" "scripts/cross-review.sh" "平台全局默认" "外部知识库" "派活单" "收工小票" "不得再派生代理")
  for a in "${anchors[@]}"; do
...
for p in cross-review spec-review code-review; do
